...
pub fn update_successor_key(node: *mut Node, diff: u8, absolute_key: u8, skipped: u32, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext) -> ReturnCode {
    if diff as usize > KEY_DELTA_STATES {
        ...
        as_top_node_mut(&mut node_ref.header).set_delta(diff);
        let free_size_left: u8 = ocx.embedded_traversal_context.root_container.as_mut().free_bytes();
        let remaining: u32 = ocx.embedded_traversal_context.root_container.as_mut().size() - (ocx.embedded_traversal_context.next_embedded_container_offset as u32 + ctx.current_container_offset as u32 + skipped + 2 + free_size_left as u32);
        unsafe {
        ...
        if ocx.embedded_traversal_context.embedded_container_depth > 0 {
            ...
            if ocx.jump_context.top_node_key < 255 {
                let current_jump_context: JumpContext = ocx.jump_context.duplicate();
                ocx.jump_context.predecessor = unsafe {
                    Some(
