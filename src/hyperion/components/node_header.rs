use crate::hyperion::components::container::{get_container_head_size, get_container_link_size, shift_container, update_space_usage, wrap_shift_container, Container, ContainerLink, EmbeddedContainer, CONTAINER_MAX_EMBEDDED_DEPTH, CONTAINER_MAX_FREESIZE};
use crate::hyperion::components::context::OperationCommand::Put;
use crate::hyperion::components::context::{ContainerTraversalContext, ContainerTraversalHeader, JumpContext, PathCompressedEjectionContext, RangeQueryContext, KEY_DELTA_STATES};
...
pub fn as_raw_char(node_head: *const NodeHeader) -> *const u8 {
    node_head as *const u8
}

pub fn as_raw_char_mut(node_head: *mut NodeHeader) -> *mut u8 {
    node_head as *mut u8
}
...
pub fn get_jump_value(node_head: *mut NodeHeader) -> u16 {
    unsafe {
        let target = (node_head as *mut u8).add(get_offset_jump(node_head));
...
pub fn set_node_value(node_head: *mut NodeHeader, ocx: &mut OperationContext) -> ReturnCode {
    if let Some(input_value) = ocx.input_value {
        ...
        unsafe {
            let offset = get_offset_node_value(node_head);
            log_to_file(&format!("offset_set_node_value: {}", offset as i32));
            let target = (node_head as *mut u8).add(offset);
            copy_nonoverlapping(input_value as *mut u8, target, size_of::<NodeValue>());
            //copy_nonoverlapping((node_head as *mut u8).add(get_offset_node_value(node_head)), input_value as *mut u8, size_of::<NodeValue>())

            log_to_file(&format!("set_node_value: {}", *target));
        }
...
pub fn call_sub_node(node_head: *mut NodeHeader, range_query_context: &mut RangeQueryContext, hyperion_callback: HyperionCallback) -> bool {
    match as_sub_node(node_head).type_flag() {
        LeafNodeEmpty => {
            hyperion_callback(range_query_context.current_key, range_query_context.current_key_offset + 2, null_mut())
        },
        LeafNodeWithValue => unsafe {
            hyperion_callback(
...
pub fn compare_path_compressed_node(node_head: *mut NodeHeader, ocx: &mut OperationContext) -> bool {
    let pc_header: &PathCompressedNodeHeader = unsafe {
        as_raw_compressed(node_head).as_ref().expect(ERR_NO_CAST_REF)
    };
    ...
    unsafe {
        ...
        memcmp(op_key as *mut c_void, key as *mut c_void, key_len as size_t) == 1
    }
}
...
pub fn use_sub_node_jump_table(node_head: *mut NodeHeader, ctx: &mut ContainerTraversalContext) -> u8 {
    ...
    if jump_class > 0 {
        ...
        ctx.current_container_offset += get_offset(node_head) as i32 + unsafe { *(jump_table_pointer.add((jump_class - 1) as usize)) } as i32;
...
pub fn safe_path_compressed_context(node_head: *mut NodeHeader, ocx: &mut OperationContext) {
    ...
    unsafe {
        ...
        if (*pc_node).value_present() {
            copy_nonoverlapping(
                (pc_node as *const u8).add(offset),
                &mut pc_ctx.node_value as *mut NodeValue as *mut u8,
                value_size,
            );
        }
...
fn get_embedded_container_info(ocx: &mut OperationContext) -> (usize, u32, u32, u8, *mut u8, *mut u8) {
    let emb_container: &mut AtomicEmbContainer = ocx.embedded_traversal_context.embedded_stack
        .as_mut().expect(ERR_EMPTY_EMB_STACK)[0]
        .as_mut().expect(ERR_EMPTY_EMB_STACK_POS);
...
fn shift_memory_after_ejection(node_head: *mut NodeHeader, ocx: &mut OperationContext, embedded_offset: usize, embedded_size: u32, shift_src_ptr: *mut u8, root_size: u32, free_bytes: u8) {
...
fn resize_root_container(ocx: &mut OperationContext, delta: i32, root_size: u32, free_bytes: u8, new_free_size_left: i32) {
    ...
    if new_free_size_left > CONTAINER_MAX_FREESIZE as i32 {
        ...
        ocx.embedded_traversal_context.root_container = get_pointer(
            ocx.arena.expect(ERR_NO_ARENA),
            ocx.embedded_traversal_context.root_container_pointer,
            1,
            ocx.chained_pointer_hook
...
pub fn add_embedded_container(mut node: *mut NodeHeader, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext) {
    ...
    let next_embedded = ocx.embedded_traversal_context.next_embedded_container.take().expect(ERR_NO_NEXT_CONTAINER);
    ...
    ocx.embedded_traversal_context.embedded_stack
        .as_mut().expect(ERR_EMPTY_EMB_STACK)[ocx.embedded_traversal_context.embedded_container_depth as usize] =
    ...
    unsafe {
        ...
        else { panic!("{}", ERR_NO_NEXT_CONTAINER) }
    }
}
...
pub fn embed_or_link_child(mut node_head: *mut NodeHeader, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext) -> *mut NodeHeader {
    ...
    let required_size_for_path_compression = size_of::<PathCompressedNodeHeader>() as i32 + ocx.key_len_left - 2
        + ocx.input_value.map_or(0, |_| size_of::<NodeValue>()) as i32;
    ...
    if as_sub_node(node_head).child_container() == PathCompressed {
        ...
        if ocx.embedded_traversal_context.embedded_container_depth == 0 {
            ...
            let embedded_size = ocx.embedded_traversal_context.embedded_stack
                .as_mut().expect(ERR_EMPTY_EMB_STACK)[0]
                .as_mut().expect(ERR_EMPTY_EMB_STACK_POS)
                .borrow_mut().size() as i32;
    ...
    match switch_condition {
        TransformPathCompressedNode => {
            ...
            node_head = unsafe {
                (ocx.embedded_traversal_context.root_container as *mut NodeHeader).add(ctx.current_container_offset as usize)
            };
        },
        CreateEmbeddedContainer => {
...
fn process_embedded_container(mut node_head: *mut NodeHeader, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext, modify: bool) -> (ReturnCode, *mut NodeHeader) {
    safe_sub_node_jump_table_context(ocx, ctx);
    let offset: usize = get_offset_child_container(node_head);
    ...
    assert!(unsafe { (*(ocx.embedded_traversal_context.next_embedded_container.expect(ERR_NO_NEXT_CONTAINER))).size() as u32 } < ocx.get_root_container().size()); // auch für !modify?
    ...
    if !modify {
        ...
        ocx.header.set_next_container_valid(EmbeddedContainerValid);
        ocx.next_container_pointer = Some(ocx.embedded_traversal_context.next_embedded_container.expect(ERR_NO_NEXT_CONTAINER) as *mut HyperionPointer);
        return (OK, null_mut());
    }
    ...
    if ocx.header.command() == Put
        && (embedded_size > embedding_hwm
        || root_size >= embedding_limit
        || (embedded_size > embedding_hwm / 2 && root_size >= embedding_limit / 2))
    {
...
pub fn get_child_container_pointer(
 ...
 pub fn create_node(
     ...
     let mut absolute_key: u8 = if embedded && container_depth == 0 { ctx.first_char } else { ctx.second_char };
     ...
     update_space_usage(if !embedded { 1 + input_memory_consumption + set_key_after_creation as usize } else { required } as i16, ocx, ctx);
     ...
     if set_key_after_creation {
         ...
         if let Some(successor) = successor_ptr.filter(|_| skipped_bytes > 0 ) {
             ...
             update_successor_key(successor as *mut Node, diff, absolute_key, skipped_bytes, ocx, ctx);
         }
     }
...
pub fn get_successor(
    node_head: *mut NodeHeader, mut successor: &mut Option<*mut Node>, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext, embedded: bool
) -> u32 {
...
pub fn create_sublevel_jumptable(mut node_head: *mut NodeHeader, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext) {
    ...
    unsafe {
        let mut offset: usize = (node_head as *mut u8)
            .offset_from(ocx.get_root_container_pointer() as *mut u8)
            as usize;
        ...
        let mut target: *mut u16 = (ocx.get_root_container_pointer() as *mut u8)
        ...
        loop {
            ...
            scan_node = (ocx.get_root_container_pointer() as *mut u8)
            ...
            if as_top_node(scan_node).container_type() == 1 {
                ...
                let offset_tmp: i32 = (scan_node as *mut u8)
                    .offset_from(ocx.get_root_container_pointer() as *mut u8)
                    as i32;
                bytes_to_move =
...
pub fn transform_pc_node(mut node_head: *mut NodeHeader, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext) {
    ...
    let pc_key_offset: usize = size_of::<PathCompressedNodeHeader>() + ocx.get_pc_ejection_context().path_compressed_node_header.value_present() as usize * size_of::<NodeValue>();
    ...
    if (ocx.get_root_container().size() >= container_embedding_limit)
        ...
        .borrow_mut()
        .size() as u32) >= (container_embedding_hwm - get_container_link_size() as u32)))
    {
        ...
        let container_tail: i32 = ocx.get_root_container().size() as i32 - (absolute_offset as i32 + child_container_offset as i32 + ocx.get_pc_ejection_context().path_compressed_node_header.size() as i32 + free_size_left as i32);
        ...
        let data_offset: *mut u8 = unsafe { (ocx.get_root_container_pointer() as *mut u8).add(ocx.get_root_container().get_container_head_size() as usize) };
        ...
        if pc_key_len == 1 {
            ...
            unsafe {
                copy_nonoverlapping(
                    ...
                    (sub as *mut u8).add(size_of::<NodeHeader>()),
                    1
                );
            }
            ...
            let mut sub: *mut NodeHeader = unsafe {
                data_offset.add(size_of::<NodeHeader>() + 1) as *mut NodeHeader
            };
            as_sub_node_mut(sub).set_container_type(1);
            ...
            let required: usize = ocx.get_pc_ejection_context().path_compressed_node_header.value_present() as usize * size_of::<NodeValue>() + remaining_pc_key_len + 1;
            consumed_newcon = 2 + 2 * size_of::<NodeHeader>() + required;
            sub = meta_expand(ocx, ctx, consumed_newcon as u32);

            let pc_node: *mut PathCompressedNodeHeader = unsafe {
                (sub as *mut u8).add(size_of::<NodeHeader>() + 1) as *mut PathCompressedNodeHeader
            };
        ...
        if pc_key_len == 1 {
            ...
            let child_container: *mut u8 = unsafe {
                (node_head as *mut u8).add(child_container_offset)
            };
            unsafe { wrap_shift_container(ocx.get_root_container_pointer(), child_container, required); }
            ...
            unsafe {
                ...
                if let Some(ref mut next_embedded_container) = ocx.embedded_traversal_context.next_embedded_container {
                    (*(*next_embedded_container)).set_size((size_of::<EmbeddedContainer>() + size_of::<NodeHeader>() + 1 + value_present * size_of::<NodeValue>()) as u8);
                }
                else { panic!("{}", ERR_NO_NEXT_CONTAINER) }
                ...
                let embedded_top: *mut NodeHeader = (node_head as *mut u8).add(child_container_offset + size_of::<EmbeddedContainer>()) as *mut NodeHeader;
            ...
            unsafe {
                ...
                if let Some(ref mut next_embedded_container) = ocx.embedded_traversal_context.next_embedded_container {
                    (*(*(next_embedded_container))).set_size((size_of::<EmbeddedContainer>() + (size_of::<NodeHeader>() + 1) * 2 + value_present * size_of::<NodeValue>()) as u8);
                }
                else { panic!("{}", ERR_NO_NEXT_CONTAINER) }
                /*unsafe { (*(ocx.embedded_traversal_context.next_embedded_container.unwrap())).set_size(
            ...
            unsafe {
                ...
                if let Some(ref mut next_embedded_container) = ocx.embedded_traversal_context.next_embedded_container {
                    (*(*(next_embedded_container))).set_size((size_of::<EmbeddedContainer>() + (size_of::<NodeHeader>() + 1) * 2 + value_present * size_of::<NodeValue>() + remaining_partial_key) as u8);
                }
                else { panic!("{}", ERR_NO_NEXT_CONTAINER) }
                ...
                let pc_node: *mut PathCompressedNodeHeader = (embedded_sub as *mut u8).add(size_of::<NodeHeader>() + 1) as *mut PathCompressedNodeHeader;
...
pub fn inject_sublevel_reference_key(node_head: *mut NodeHeader, ocx: &mut OperationContext, ctx: &mut ContainerTraversalContext, refkey: u8) {
    ...
    unsafe {
        let node_offset: i32 = (node_head as *mut u8)
            .offset_from(ocx.get_root_container_pointer() as *mut u8)
            as i32;
        ...
        write_bytes(node_head as *mut u8, 0, (size_of::<NodeHeader>() + 1).wrapping_sub(relative as usize));
        as_top_node_mut(node_head).set_type_flag(InnerNode);
    ...
    if relative == 0 {
        ...
        as_top_node_mut(node_head).set_delta(diff % 8);
